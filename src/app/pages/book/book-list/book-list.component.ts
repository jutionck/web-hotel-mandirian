...
export class BookListComponent implements OnInit, IBookListComponent {
  ...
  onReserve(booking: Book): void {
    if (booking.status === 'checked-in' || booking.status === 'checked-out') {
      alert(
        `Tamu ${booking.reserve.name} sudah melakukan ${booking.status} tidak bisa ubah durasi malam`
      );
    } else {
    ...
    this.hotelService.get(bookingId).subscribe({
      next: (booking: Book) => {
        if (booking.status === 'checked-out') {
          alert(
            `Tamu ${booking.reserve.name} sudah ${booking.status} tidak bisa melakukan checked-in`
          );
        } else {
          this.hotelService.checkIn(bookingId).subscribe();
          alert(
            `Tamu ${booking.reserve.name} sudah check-in pada kamar ${booking.roomNumber}.`
          );
        }
      },
    });
    ...
    this.hotelService.get(bookingId).subscribe({
      next: (booking: Book) => {
        if (booking.status === 'reserved') {
          alert(
            `Tamu ${booking.reserve.name} belum melakukan checked-in pada kamar ${booking.roomNumber}.`
          );
        } else {
          this.hotelService.checkOut(bookingId).subscribe();
          alert(
            `Tamu ${booking.reserve.name} sudah check-out pada kamar ${booking.roomNumber}.`
          );
        }
      },
    });
  ...
  onDeleteReservation(bookingId: number): void {
    this.hotelService.get(bookingId).subscribe({
      next: (booking: Book) => {
        if (booking.status !== 'checked-out') {
          alert(
            `Data pemesanan tidak dapat di hapus karena tamu ${booking.reserve.name} belum checkout.`
          );
        } else {
